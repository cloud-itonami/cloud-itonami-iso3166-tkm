(ns marketentry.store
  "SSoT for the TKM market-entry compliance actor, behind a `Store`
  ...
  Unlike AGO's/LBY's engagement record, this one carries NO
  jurisdiction-specific conditional-check ground-truth fields (no
  `:requires-ao-entity?`/`:has-ao-entity?`-style pair, no bank-balance
  field) -- TKM's genuinely thin, low-confidence fact base does not
  support an additional flagship governor conditional check (see
  `marketentry.governor`/`marketentry.facts` docstrings), so the
  engagement schema stays honest about what it actually verifies:
  identity, portal, fee math, and the double-actuation guards.
...
(defn demo-data
  ...
  lifecycles (draft, submit) plus the fee-mismatch and double-actuation
  guard scenarios. Deliberately smaller than AGO's/LBY's demo set --
  TKM has no jurisdiction-specific conditional check for a demo
  scenario to exercise (see namespace docstring)."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems TM" :portal "State Tender Registry"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :drafted? false :submitted? false
             :jurisdiction "TKM" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "State Tender Registry"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :drafted? false :submitted? false
             :jurisdiction "ATL" :status :intake}
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "State Tender Registry"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
