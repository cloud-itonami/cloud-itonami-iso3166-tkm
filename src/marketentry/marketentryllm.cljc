(ns marketentry.marketentryllm
  ...
  the TKM public-sector market-entry compliance actor.
  ...
  end-to-end. `propose-submit`'s confidence heuristic here is simpler
  than AGO's/LBY's siblings' -- it has no jurisdiction-specific
  ground-truth field to peek at (TKM's fact base does not support one,
  see `marketentry.facts`/`marketentry.governor`), so confidence
