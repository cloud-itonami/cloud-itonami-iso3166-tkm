(ns marketentry.sim
  ...
  commit), then shows HARD-hold scenarios. Deliberately smaller than
  AGO's/LBY's demo -- TKM has no jurisdiction-specific conditional
  check to exercise (see `marketentry.governor`/`marketentry.facts`)."
...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (TKM, clean) ==")
    ...
    (println (exec-op actor "t8" {:op :filing/draft :subject "eng-1"} operator))

    (println "== filing/submit eng-1 AGAIN (double-submit -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-1"} operator))
