...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X Co" :jurisdiction "TKM"
  ...
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "TKM" :checklist ["a"] :spec-basis "x"}})
...
(deftest seed-db-has-demo-engagements
  (let [db (store/seed-db)]
    (is (= 3 (count (store/all-engagements db))))
    (is (some? (store/engagement db "eng-1")))))
